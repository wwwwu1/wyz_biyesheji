		            ...
		            <div class="col-12">
		                <div class="sw_no">
							...
							<div class="card_notice">
								<div class="notice_title">公告通知 <router-link to="notice/list">更多</router-link></div>
								<!-- 公告组件 -->
		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐动漫交流模块(开始) -->
		<div class="warp" v-if="$check_action('/exchange_forum/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/exchange_forum/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_exchange_forum = !show_list_exchange_forum">
		                        <div class="switch_box">
		                            <span v-if="show_list_exchange_forum"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 动漫交流推荐列表组件 -->
		                <list_exchange_forum v-if="show_list_exchange_forum" :list="list_exchange_forum" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_exchange_forum.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_exchange_forum" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_exchange_forum" :key="i" @click="to_details('exchange_forum',o,'exchange_forum_id')">
		                            <td v-for="(oj,n) in fields_exchange_forum">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
							show_list_exchange_forum: true,
				list_exchange_forum: [],
				vm_arr_exchange_forum: [
					"exchange_forum_id",
							"forum_title",
							"forum_type",
							"forum_tag",
		...
		methods: {
			...
			get_exchange_forum() {
				let url = "~/api/exchange_forum/get_list?";
								// url = "~/api/exchange_forum/get_list?orderby=hits desc";
								this.$get(url, {
		...
		computed: {
					    list_table_sign_in_information() {
