			...
			<div class="container">
				...
				<div class="row">
					...
					<div class="details_btn_wrap">
											<button class="btn btn-primary details_btn" v-if="$check_action('/reference_information/edit','add')" @click="to_form('/reference_information/edit')" > 参考 </button>
...
<script>
	...
	export default {
      ...
      check_collect() {
        var user_id = this.$store.state.user.user_id;
        var learning_materials_id = this.obj.learning_materials_id;
        this.$get('~/api/collect/count?', {
          user_id,
          source_table: "learning_materials",
          source_field: "learning_materials_id",
          source_id: learning_materials_id
        }, (res) => {
